...
export default function ProblemSection() {
  return (
    <section className="relative flex w-full flex-col items-center bg-[#FBFBFA] px-6 pt-0 pb-0 sm:px-12 sm:pt-0 sm:pb-0">
      {/* Diagonal blueprint lines (constrained to main column border) */}
      <div className="relative w-full h-12 overflow-visible">
        {/* Diagonal lines overlay (breaks out to fill full 100vw viewport width) */}
      ...
      {/* Dashed Grid Title Box Wrapper (with masking to remove vertical borders only for this section) */}
      <div className="relative w-full flex justify-center">
        {/* Left vertical border mask */}
        <div className="absolute -left-[1px] top-0 bottom-0 w-[2px] bg-[#FBFBFA] z-30"></div>
        {/* Right vertical border mask */}
        <div className="absolute -right-[1px] top-0 bottom-0 w-[2px] bg-[#FBFBFA] z-30"></div>
      ...
      {/* Lower diagonal blueprint lines */}
      <div className="relative w-full h-12 overflow-visible">
        {/* Diagonal lines overlay (breaks out to fill full 100vw viewport width) */}
        <div className="absolute inset-y-0 left-1/2 -translate-x-1/2 w-[100vw] pointer-events-none">
          <svg className="w-full h-full text-neutral-200/80" fill="none">
            <defs>
              <pattern id="problem-diagonal-hatch-bottom" width="12" height="48" patternUnits="userSpaceOnUse">
                <line x1="0" y1="0" x2="12" y2="48" stroke="currentColor" strokeWidth="1" />
              </pattern>
            </defs>
            <rect width="100%" height="100%" fill="url(#problem-diagonal-hatch-bottom)" />
          </svg>
          {/* Bottom horizontal line running full viewport width */}
          <div className="absolute bottom-0 left-0 right-0 border-b border-neutral-200/80"></div>
        </div>
      </div>

      {/* Comparison Grid */}
      <div className="-mx-6 sm:-mx-12 mt-0 grid w-[calc(100%+3rem)] sm:w-[calc(100%+6rem)] max-w-7xl grid-cols-1 gap-0 lg:grid-cols-2">
        {/* Without PipeLine — clean column */}
        <div className="flex flex-col pb-8 border-b border-neutral-200 lg:pb-12 lg:border-b-0 lg:border-r lg:border-neutral-200">
          <div className="flex flex-col gap-1 pt-8 lg:pt-12 pb-4 border-b border-neutral-200 px-6 sm:px-12 lg:px-8">
            <h3 className="text-2xl font-normal text-neutral-900">Without PipeLine</h3>
            <p className="text-sm text-gray-600">Missed calls. Guesswork. Lost jobs.</p>
          </div>
          <div className="mt-6 flex flex-col gap-3 px-6 sm:px-12 lg:px-8">
            {OLD_WAY.map((item) => (
              <div key={item} className="flex items-start gap-3 rounded-none border border-neutral-200 bg-white px-4 py-3">
        ...
        {/* With PipeLine — clean column */}
        <div className="flex flex-col pt-8 pb-12 lg:pt-0 lg:pb-12">
          <div className="flex flex-col gap-1 pt-8 lg:pt-12 pb-4 border-b border-neutral-200 px-6 sm:px-12 lg:px-8">
            <h3 className="text-2xl font-normal text-neutral-900">
              With <span className="bg-[#FAFB86] px-2 rounded-sm inline-block">PipeLine</span>
            </h3>
            <p className="text-sm text-gray-600">Answered. Booked. Handled.</p>
          </div>
          <div className="mt-6 flex flex-col gap-3 px-6 sm:px-12 lg:px-8">
            {PIPELINE_WAY.map((item) => (
              <div key={item} className="flex items-start gap-3 rounded-none border border-lime-200 bg-lime-50/20 px-4 py-3">
